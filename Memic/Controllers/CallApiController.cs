...
namespace Memic.Controllers
{
    public class CallApiController : Controller
    {
        ...
        private async Task<TokenResponse> GetTokenAsync()
        {
            ...
            var client = new TokenClient(IdentityServerConfig.Endpoint.IdentityServerTokenUri,
            "mvc_service",
            "secret");

            return await client.RequestClientCredentialsAsync("memicApi");
        }
        private async Task<string> CallApi(string token)
        {
            var client = new HttpClient();
            client.SetBearerToken(token);
            var json = await client.GetStringAsync(IdentityServerConfig.Endpoint.APIIdentityUri);
